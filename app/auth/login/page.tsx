...
export default function Login() {
  ...
  const ADMIN_EMAIL = "vedaskudalkar@gmail.com"; // Replace with the actual admin email
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (user.email === ADMIN_EMAIL) {
        router.push('/adminpanel/dashboard'); // Redirect to Admin Panel
      } else {
        router.push('/patientpanel/dashboard'); // Redirect to User Dashboard
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-md mt-10 w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-black dark:bg-gray-900">
      ...
      <p className="text-neutral-600 text-sm max-w-sm mt-2 dark:text-neutral-300">
        Login to manage your health profile and access care services tailored for you.
      </p>
